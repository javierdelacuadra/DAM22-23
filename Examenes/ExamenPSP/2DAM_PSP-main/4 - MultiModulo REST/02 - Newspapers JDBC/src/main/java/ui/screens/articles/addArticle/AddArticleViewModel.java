    ...
    public void updateArtTable(int index) {
        if (index > _state.get().getNewspaperList().size() || index < 0) {
            ...
            if (articlesByPaper.isRight()) {
                ...
                jn = new AddArticleState(articlesByPaper.getLeft(), new ArrayList<>(), _state.getValue().getNewspaperList(), _state.getValue().getTypeArtList(), null);
            }
            _state.setValue(jn);
        }
    }
    ...
    public void addArticle(String text, String desc, int indexType, int indexNews) {
        ...
        if (text.equals("")) {
            ...
            if (articlesInTheNews.isRight()) {
                ...
                jn = new AddArticleState(articlesInTheNews.getLeft(), null, _state.getValue().getNewspaperList(), _state.getValue().getTypeArtList(), null);
            }
        }
        _state.setValue(jn);
    }
