...
public class ReadersDAOImpl implements ReadersDAO {
    ...
    // >0 -> List of readers
    // NumericConstants.NON_RELATED_TO_DB_EXCEPTION_CODE -> error related not related to de DB (Exception e)
    // NumericConstants.DB_EXCEPTION_CODE -> error with the transaction connection related to de DB (SQLException e)
    @Override
    ...
    private int updateTransaction(Reader reader) {
        ...
        try (Connection con = dbConnection.getConnection()) {
            ...
            try (PreparedStatement updReader = con.prepareStatement(SQLQueries.UPDATE_READERS_SET);
                ...
                try {
                    con.rollback();
                    if (sqle.getErrorCode() == NumericConstants.SQL_INTEGRITY_CONSTRAINT_VIOLATION_EXCEPTION) {
