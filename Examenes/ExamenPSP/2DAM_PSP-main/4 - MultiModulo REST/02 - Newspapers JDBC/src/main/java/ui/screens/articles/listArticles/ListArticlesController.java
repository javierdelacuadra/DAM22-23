...
public class ListArticlesController extends BaseScreenController {
    ...
    @FXML
    private MFXTableView<QueryArticlesAndNews> tblArticlesNews;
    ...
    public void principalCargado() {
        ...
        MFXTableColumn<QueryArticlesAndNews> nameArt = new MFXTableColumn<>(ScreenConstants.NAME, true, Comparator.comparing(QueryArticlesAndNews::getName_article));
        MFXTableColumn<QueryArticlesAndNews> nameNews = new MFXTableColumn<>(ScreenConstants.NEWSPAPER, true, Comparator.comparing(QueryArticlesAndNews::getName_newspaper));
        nameArt.setRowCellFactory(query -> new MFXTableRowCell<>(QueryArticlesAndNews::getName_article));
        nameNews.setRowCellFactory(query -> new MFXTableRowCell<>(QueryArticlesAndNews::getName_newspaper));
        tblArticlesNews.setFooterVisible(false);
        tblArticlesNews.getTableColumns().addAll(nameArt, nameNews);
    ...
    private void addListener() {
        listArticlesViewModel.getState().addListener((observableValue, listadoState, listadoStateNew) -> {
            ...
            if (listadoStateNew.getQueryArticlesAndNewsList() != null) {
                tblArticlesNews.getItems().clear();
                tblArticlesNews.getItems().addAll(listadoStateNew.getQueryArticlesAndNewsList());
            }
