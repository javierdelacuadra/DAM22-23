...
public class LoginController extends BaseScreenController {
    ...
    private void addListenerState() {
        loginViewModel.getState().addListener((observable, principalState, principalStateNew) ->
                Platform.runLater(() -> {
                    if (principalStateNew.getUserLogged() != null) {
                        getPrincipalController().onLoginHecho(principalStateNew.getUserLogged());
                    }
        ...
        mfxUser.textProperty().addListener((observable, oldValue, newValue) -> mfxPassword.setText(newValue));
        mfxUser.setOnAction(event -> tryLogin());
        mfxPassword.setOnAction(event -> tryLogin());
    }
