...
public class AuthorizationInterceptor implements Interceptor {
    ...
    private Request doLogin(Request original) {
        Request request;
        String url = original.url().toString().substring(0, original.url().toString().indexOf(APIConstants.BASE_API_URL_PATH) + APIConstants.BASE_API_URL_PATH.length()-1) + AplicationConstants.PATH_USERS + AplicationConstants.PATH_LOGIN;
        request =
        original.newBuilder()
                .url(url)
                .post(RequestBody.create(MediaType.get(APIConstants.APPLICATION_JSON), ca.getUserLogged().toJsonString()))
                .build();
        return request;
    }
}
