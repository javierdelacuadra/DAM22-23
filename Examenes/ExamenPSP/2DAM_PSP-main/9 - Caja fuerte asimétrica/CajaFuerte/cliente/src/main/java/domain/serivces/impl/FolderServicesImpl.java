...
public class FolderServicesImpl implements FoldersServices {
            ...
            if (f.isRight()) {
                try {
                    if (encriptacionAsimetrica.desencriptarBase64ConClavePrivada(f.get().getPassword(), ca.getPrivateKeyUserLogged()).equals(passFolder)) {
                        ...
                        callFuture.complete(Single.just(Either.left(ServicesConstants.CARPETA_O_CONTRASENA_INCORRECTA)));
                    }
                } catch (Exception e) {
                    callFuture.complete(Single.just(Either.left(ServicesConstants.NO_TIENES_PERMISOS_PARA_ACCEDER_A_ESTA_CARPETA)));
                }
            } else {
                callFuture.complete(Single.just(f));
            }
    ...
    public Single<Either<String, Response<Object>>> add(Folder f) {
        try {
            ...
            return Single.just(Either.left(ServicesConstants.ERROR_AL_ANADIR_LA_CARPETA));
        }
    }
