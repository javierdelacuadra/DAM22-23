...
public class EncriptAsim implements EncriptacionAsimetrica {
    ...
    public static final String NO_SE_HA_PODIDO_GUARDAR_EL_CERTIFICADO_Y_LA_CLAVE_PRIVADA_EN_EL_KEY_STORE = "No se ha podido guardar el certificado y la clave privada en el KeyStore";
    ...
    public static final String NO_SE_HA_PODIDO_CREAR_EL_KEY_STORE = "No se ha podido crear el KeyStore";
    public static final String NO_SE_HA_PODIDO_OBTENER_LA_CLAVE_PUBLICA_DEL_KEY_STORE = "No se ha podido obtener la clave pública del KeyStore";
    ...
    /**
     * Método que obtiene un KeyStore a partir de una ruta y una contraseña
     *
    ...
    public Either<String, KeyStore> getKeyStore(String pathKeyStore, String password) {
        ...
        try (FileInputStream fileInputStream = new FileInputStream(pathKeyStore)) {
            keyStore = KeyStore.getInstance(AplicationConstants.PKCS_12);
            ...
            return Either.left(USUARIO_O_CONTRASENA_INCORRECTOS);
        }
    ...
    /**
     * Método que obtiene la clave pública de un string codificado en Base64
     *
    ...
    /**
     * @param strToEncrypt String a encriptar (En caso del registro debería ser el String con el que ciframos simétricamente la PrivateKey del usuario)
     * @param aPublic      PublicKey con la que se encriptará (En caso del registro debería ser la PublicKey del servidor)
    ...
    public void createKeyStore(char[] secretKey, PrivateKey aPrivate, String certificateBase64, Path path) {
        ...
        try {
            ...
            throw new RuntimeException(NO_SE_HA_PODIDO_GUARDAR_EL_CERTIFICADO_Y_LA_CLAVE_PRIVADA_EN_EL_KEY_STORE);
        }
        ...
        try (OutputStream fos = new FileOutputStream(path.toFile())) {
            ...
            throw new RuntimeException(NO_SE_HA_PODIDO_GUARDAR_EL_KEY_STORE_EN_EL_FICHERO);
        }
    }
    ...
    private KeyStore generateKeyStoreInstance() {
        KeyStore ks;
        try {
            ks = KeyStore.getInstance(AplicationConstants.PKCS_12);
            ks.load(null, null);
        } catch (KeyStoreException | CertificateException | IOException | NoSuchAlgorithmException e) {
            throw new RuntimeException(NO_SE_HA_PODIDO_CREAR_EL_KEY_STORE);
        }
        return ks;
    }
     ...
     * @return true si la firma es correcta, excepción en caso contrario
     */
    @Override
    public boolean verificarfirma(PublicKey publicKey, Firma firma) throws NoSuchAlgorithmException, InvalidKeyException, SignatureException {
        ...
        signature.update(firma.getTextoOriginal().getBytes());
        return signature.verify(Base64.getUrlDecoder().decode(firma.getTextoFirmado()));
    }
    ...
    public PublicKey getPublicKeyFromKeyStore(KeyStore keyStore) {
        try {
            ...
            throw new RuntimeException(NO_SE_HA_PODIDO_OBTENER_LA_CLAVE_PUBLICA_DEL_KEY_STORE);
        }
    }
}
