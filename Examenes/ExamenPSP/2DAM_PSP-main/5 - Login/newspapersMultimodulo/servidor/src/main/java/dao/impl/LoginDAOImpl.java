...
public class LoginDAOImpl implements LoginDAO {
    ...
    @Inject
    public LoginDAOImpl(DBConnection dbConnection, SendEmail sendEmail, Pbkdf2PasswordHash passwordHash) {
    ...
    private boolean resetTime(String token, Connection con) {
            ...
            if (rs.next()) {
                ...
                if (!valido) {
                    ...
                    if (LocalDateTime.now().isAfter(fechaRegistro.plusMinutes(5))) {
                        ...
                        updateValido.setString(2, email);
                        updateValido.executeUpdate();
                        sendEmail.generateAndSendEmail(email, DAOConstants.MAIL_BIENVENIDA.formatted(token, token), DAOConstants.RE_VALIDAR_USER_NEWSPAPERS);
                    } else {
        ...
        try (Connection con = dbConnection.getConnection()) {
            ...
            try (PreparedStatement insertReader = con.prepareStatement(SQLQueries.INSERT_INTO_READERS, Statement.RETURN_GENERATED_KEYS);
                ...
                if (rs.next()) {
                    ...
                    sendEmail.generateAndSendEmail(newReader.getLogin().getEmail(), DAOConstants.MAIL_BIENVENIDA.formatted(token, token), DAOConstants.BIENVENIDO_A_LOS_NEWSPAPERS);
    ...
    @Override
    public boolean validForgot(String email) {
        try (Connection con = dbConnection.getConnection();
            ...
            if (rs.next()){
                ...
                sendEmail.generateAndSendEmail(email, DAOConstants.MAIL_RESET_PASS.formatted(token), DAOConstants.CAMBIA_TU_CONTRASENA_NEWSPAPERS);
                return true;
            }
