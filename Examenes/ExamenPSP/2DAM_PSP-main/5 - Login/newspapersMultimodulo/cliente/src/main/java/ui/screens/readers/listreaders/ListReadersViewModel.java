...
public class ListReadersViewModel {
    ...
    public void deleteReader(Reader killedReader, Reader readerLoggedIn) {
            ...
            readerServicesImpl.delete(_state.getValue().getReaderSelected())
                    ...
                    .subscribe(readerDeleteSuccess -> {
                        ...
                        if (readerDeleteSuccess.isRight()) {
                            ...
                            String mensajeDeleted = checkIfDeletedIsLogged(killedReader, readerLoggedIn);
                            _state.setValue(ListReadersState.builder()
                                    .error(null)
                                    .readers(currentAll)
    ...
    private String checkIfDeletedIsLogged(Reader killedReader, Reader readerLoggedIn) {
        // dependiendo si se elimino a si mismo se saca un mensaje u otro
        String mensajeDeleted;
        if (killedReader.getId() == readerLoggedIn.getId()) {
            mensajeDeleted = ScreenConstants.YOU_HAVE_DELETED_YOURSELF;
        } else {
            mensajeDeleted = ScreenConstants.SE_ELIMINO_EL_LECTOR + killedReader.getName();
        }
        return mensajeDeleted;
    }
