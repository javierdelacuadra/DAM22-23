...
public class ReaderServicesImpl implements ReaderServices {
    ...
    // -3 -> error with the transaction, rollback failed
    ...
    //codes:
    //-2 -> error unconnected with the DB
    ...
    public Either<String, Reader> add(Reader newReader) {
        ...
        if (newReader.getName().equals("") || newReader.getBirthDate() == null || newReader.getLogin().getUser().equals("") || newReader.getLogin().getPassword().equals("")) {
            ...
            switch (code) {
                case -3 -> either = Either.left(ServicesConstants.USER_ALREADY_EXISTS);
                case -2 -> either = Either.left(ServicesConstants.ERROR_ADDING_READER + newReader.getName()
    ...
    public Either<String, Reader> update(Reader newReader) {
        ...
        if (newReader.getName().equals("") || newReader.getBirthDate() == null
            ...
            switch (code) {
                case -2 -> either = Either.left(ServicesConstants.ERROR_UPDATING_READER + ServicesConstants.BREAK + newReader.getName()
                        + ServicesConstants.NON_RELATED_WITH_THE_DB);
                case -3, -1 -> either = Either.left(ServicesConstants.ERROR_UPDATING_READER + newReader.getName()
                ...
                case 1062 -> either = Either.left(ServicesConstants.ERROR_UPDATING_READER + newReader.getName()
    ...
    public Either<String, List<String>> getOldestReaders(Newspaper newspaperSelected) {
        ...
        if (listReaders.isRight()) {
            ...
            switch (listReaders.getLeft()) {
                ...
                case -3 -> returnEither = Either.left(ServicesConstants.ERROR_GETTING_READERS_FROM_THE_DB);
                default ->
    ...
    private Either<String, List<Reader>> getGetAllEither(Either<Integer, List<Reader>> listReaders) {
        ...
        if (listReaders.isRight()) {
            ...
            switch (listReaders.getLeft()) {
                ...
                case -3 -> either = Either.left(ServicesConstants.ERROR_GETTING_READERS_FROM_THE_DB);
                default ->
