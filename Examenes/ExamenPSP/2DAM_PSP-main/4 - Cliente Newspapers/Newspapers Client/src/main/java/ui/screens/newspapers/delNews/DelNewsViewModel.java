...
public class DelNewsViewModel {
    ...
    public void newspaperValid(Newspaper news) {
        Either<String, Newspaper> newspaperDeleteEither = newspapersServicesImpl.get(news);
        if (newspaperDeleteEither.isRight()) {
            _state.setValue(new DelNewsState(null, _state.getValue().getArticlesList(), _state.get().getNewspaperList(), newspaperDeleteEither.get(), ScreenConstants.ARE_YOU_SURE_YOU_WANT_TO_DELETE_THIS_READER_WITH_ALL_ITS_READ_ARTICLES_AND_LOGIN, null));
        } else {
            _state.setValue(new DelNewsState(newspaperDeleteEither.getLeft(), null, _state.get().getNewspaperList(), null, null, null));
        }
    }

    public void deleteNewspaper() {
        Either<String, String> newspaperDeleteSuccess = newspapersServicesImpl.delete(_state.getValue().getNewspaperSelected());
        if (newspaperDeleteSuccess.isRight()) {
            ...
            _state.setValue(new DelNewsState(null, null, currentNews, null, null, newspaperDeleteSuccess.get()));
        } else {
            _state.setValue(new DelNewsState(newspaperDeleteSuccess.getLeft(), null, null, null, null, null));
        }
    }
}
