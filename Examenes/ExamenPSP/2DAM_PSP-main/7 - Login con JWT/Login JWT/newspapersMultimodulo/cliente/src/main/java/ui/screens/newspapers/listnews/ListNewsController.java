...
public class ListNewsController extends BaseScreenController implements Initializable {
    ...
    @FXML
    private MFXButton btnOldestReaders;
    ...
    public void principalCargado() {
        Reader readerLogged = getPrincipalController().getReaderLoggedIn();
        if (readerLogged == null) {
            btnDelete.setVisible(false);
            btnOldestReaders.setVisible(false);
            tblReaders.setVisible(false);
        } else {
            if (readerLogged.getId() != CommonConstants.ID_ADMIN) {
                if (readerLogged.getId() == CommonConstants.ID_INVITADO) {
                    // si es invitado, no puede ver oldest readers (funcionalidad valida para readers y admins)
                    btnOldestReaders.setVisible(false);
                }
                btnDelete.setVisible(false);
            }
