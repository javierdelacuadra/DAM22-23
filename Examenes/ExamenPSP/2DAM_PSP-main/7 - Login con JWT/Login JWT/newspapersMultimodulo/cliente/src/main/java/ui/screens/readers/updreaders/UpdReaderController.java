...
public class UpdReaderController extends BaseScreenController {
    ...
    private void checkReaderSelected(UpdReaderState updReaderStateNew) {
        if (updReaderStateNew.getReaderSelected() != null) {
            ...
            if (actualReader.getLogin() != null) {
                txtUser.setText(actualReader.getLogin().getEmail());
            }
    ...
    public void clearTblSelection(){
        updReaderViewModel.clearReader();
    }
    public void updateReader() {
        updReaderViewModel.updateReader(txtName.getText(), dpkBirth.getValue());
    }
