...
public class LoginDAOImpl implements LoginDAO {
    ...
    @Inject
    public LoginDAOImpl(DBConnection dbConnection, Pbkdf2PasswordHash passwordHash, Configuration configuration) {
    ...
    @Override
    public boolean changePetitonsStatus(String email){
        try(Connection con = dbConnection.getConnection();
            PreparedStatement preparedStatement = con.prepareStatement(SQLQueries.GET_PETITION_STATUS_BY_EMAIL);
            PreparedStatement updPetition = con.prepareStatement(SQLQueries.UPDATE_PETITION_STATUS_BY_EMAIL))   {
            preparedStatement.setString(1, email);
            ResultSet rs = preparedStatement.executeQuery();
            if (rs.next()){
                int peticionesRestantes = rs.getInt(1);
                LocalDateTime fecharReinicioPeticiones = rs.getObject(2, LocalDateTime.class);
                if (fecharReinicioPeticiones.plusHours(configuration.getRequestsHoursTimeout()).plusMinutes(configuration.getRequestsMinutesTimeout()).plusSeconds(configuration.getRequestsSecondsTimeout()).isBefore(LocalDateTime.now())){
                    // ya pasó el límite, se reinicia hora y peticiones restantes
                    updPetition.setInt(1, configuration.getMaxReaderPetitions() - 1);
                    updPetition.setObject(2, LocalDateTime.now());
                }
                else {
                    if (peticionesRestantes > 0){
                        // no llegó al límite, se resta una petición
                        updPetition.setInt(1, peticionesRestantes - 1);
                        updPetition.setObject(2, fecharReinicioPeticiones);
                    }
                    else {
                        // no hay peticiones restantes
                        throw new TooManyRequestException(DAOConstants.LIMITE_DE_PETICIONES_SUPERADO +  LocalDateTime.now().until(fecharReinicioPeticiones.plusHours(configuration.getRequestsHoursTimeout()).plusMinutes(configuration.getRequestsMinutesTimeout()).plusSeconds(configuration.getRequestsSecondsTimeout()), ChronoUnit.SECONDS) + RestConstants.SEGUNDOS);
                    }
                }
                updPetition.setString(3, email);
                return updPetition.executeUpdate() == 1;
            }
            throw new NotFoundException(DAOConstants.NO_SE_ENCONTRO_UN_USUARIO_CON_ESE_EMAIL_ESTAS_SEGURO_QUE_ES_CORRECTO);
